	...
	renderOverview: function() {
		...
		s.renderRowActions = function(section_id) {
			...
			if (inst.getWifiNetworks) {
				...
				//Daniel Barba 20230501
				//works with shellyutilities.lua api - disables UI remove and edit features for default interface(reserved for shelly utilities)
				//prevents changing of wireless config file for sed commands in scripting for restoring or setting wireless config
				if( String(inst.getIfname())== "wlan0-743559" || String(inst.getIfname())== "radio0.network1"){
					btns = [
						E('button', {
		...
		s.handleAdd = function(radioDev, ev) {
			...
			uci.set('wireless', section_id, 'device', radioDev.getName());
			uci.set('wireless', section_id, 'mode', 'ap');
			uci.set('wireless', section_id, 'ssid', 'Shelly-WiFi');
			uci.set('wireless', section_id, 'encryption', 'none');
		...
		//Daniel Barba 20230518 removed asociated stations, something from previous mod does not show current connections
		//this feature is not needed, original: return E([ nodes, E('h3', _('Associated Stations')), table ]);
			return E([nodes]);
